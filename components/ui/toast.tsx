...
function createToast(options: {
  ...
  const manager = toastManager as any;
  
  try {
    // base-ui Toast manager должен иметь метод для создания тостов
    // Проверяем все возможные варианты API
    
    // Вариант 1: createToast (наиболее вероятный)
    if (typeof manager.createToast === 'function') {
      manager.createToast(options);
      return;
    }
    ...
    // Вариант 4: addToast
    if (typeof manager.addToast === 'function') {
      manager.addToast(options);
      return;
    }
    
    // Вариант 5: может быть это store с методами get/set
    if (typeof manager.get === 'function' && typeof manager.set === 'function') {
      const currentToasts = manager.get() || [];
      ...
      manager.set([...currentToasts, toastData]);
      return;
    }
    
    // Вариант 6: может быть это объект со свойством toasts (массив)
    if (manager.toasts && Array.isArray(manager.toasts)) {
      const id = `toast-${Date.now()}-${Math.random()}`;
      const toastData = {
        id,
        title: options.title,
      ...
      manager.toasts.push(toastData);
      // Попробуем вызвать обновление, если есть метод notify или update
    ...
    // Если ничего не сработало, выводим отладочную информацию
    console.warn('[Toast] Available methods:', Object.keys(manager || {}));
    console.warn('[Toast] Manager type:', typeof manager);
    console.warn('[Toast] Manager value:', manager);
    console.error('[Toast] Unable to create toast. Options:', options);
    
  } catch (error) {
    console.error('[Toast] Error creating toast:', error);
